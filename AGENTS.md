...
Deployment is automatic: GitHub Actions publishes to GitHub Pages on push to `main` or the active `Codex/*` branch. Because the app is served from the `/PausePump/` sub-path, **all asset references must be relative** (`./...`), especially in `sw.js`.

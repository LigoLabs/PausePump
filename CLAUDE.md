...
### Notifications
Shown only while `document.hidden` (guarded in `showTimerNotification`/`showFinishNotification`). The timer notification is re-issued every second (`maybeNotify` keyed on `lastNotifSec`) to act as a live countdown. On return to foreground, notifications are cleared aggressively (`clearAllNotificationsSoon`, plus a `CLEAR_NOTIFS` postMessage to the service worker, which is the most reliable way to close them). The notification **`badge`** must be a transparent monochrome PNG (`icons/badge-96.png`) — a fully opaque icon renders as a white square in the Android status bar.
